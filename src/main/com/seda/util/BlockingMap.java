...
public class BlockingMap<K, V> {
	...
	private class Entry<T> {
		...
		public String toString() {
			return "Entry[" + object + ",Latch count="
		...
		public T getObject(long timeout, TimeUnit unit)
			...
			if (this.object != null) {
				...
				BlockingMap.this.numberOfHitsWithWaiting++;
				boolean successfullCountdownWithNoTimeout = this.latch.await(
						timeout, unit);
	...
	public boolean putIfAbsent(K key, V value) {
		if (key == null || value == null) {
			throw new IllegalArgumentException("Nulls in Key/Value -> " + key
					+ ":" + value);
		}
	...
	public boolean _putIfAbsent(K key, V value) {
		synchronized (this.map) {
			...
			/*
			 * If there is an entry already this means that there's someone
			 * waiting - so we add the value and release/countdown the latch.
			 */
			if (entry != null) {
				{
					if (entry.isEmpty()) {
	...
	@Override
	public String toString() {
		String msg = "Size=" + this.size() + "; Insertions="
				+ this.numberOfInsertions + "; Removals="
				+ this.numberOfRemovals + "; " + "Latches used="
				+ this.numberOfCountDownLatches + "; Hits with no wait="
				+ this.numberOfNonWaitHits + "; Hits with waits="
				+ this.numberOfHitsWithWaiting;
		return msg;
	}
	...
	/**
	 * Will BLOCK if value doesn't exist, i.e., until an entry is made with this
	 * key by a separate thread.
	 * 
	 * @param key
	 * @return
	 * @throws InterruptedException
	...
	/**
	 * Will BLOCK for timeout if value doesn't exist, i.e., until an entry is
	 * made with this key by a separate thread.
	 * 
	 * @param key
	 * @param timeout
	 * @param unit
	...
	public static void main(String[] args) throws InterruptedException {
		...
		final BlockingMap<String, String> map2 = new BlockingMap<String, String>(
				10);
		...
		for (int i = 0; i < nThreads; i++) {
			...
			Thread t = new Thread() {
				...
				public void run() {
					try {
						String key = "T1." + j;
						log("Requesting item w/ key '" + key + "' for Thread "
								+ this.getName());
						// String string = map2.get(key);
						// print("Received item w/ key '" + key + "' -> " +
						// string + " for Thread " + this.getName());
						Future<String> stringF = map2.getFuture(key);
						log("Received item w/ key '" + key + "' -> "
								+ stringF.get() + " for Thread "
								+ this.getName());
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			Thread t2 = new Thread() {
				...
				public void run() {
					...
					log("Received item w/ key '" + key + "' -> " + string
							+ " for Thread " + this.getName());
				}
			};
			t2.start();
		}
	 ...
	 * @param key
	 * @param returnValue
	 *            the value to return upon successful completion.
